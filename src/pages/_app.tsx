        ...
        <ul className="flex justify-center lg:block">
          {
            menu.map((item, i) =>
              <li key={i}>
                <Link
                  href={item.path}
                  className={(path === item.path ? clsx(baseClassNames, item.activityClass) : clsx(baseClassNames, item.hoverClass))}>
                  <item.icon
                    className="mb-1 lg:mr-2 lg:mb-0"
                    width={24}
                    fill={path === item.path ? item.activityIconColor : item.iconColor}
                  />
